...
const BASE_URL = __ENV.BASE_URL || "https://www.pp01.test.ikea.cn/pvid/";
...
// Note: Always using imwidth format for URLs
...
const LARGE_FILE_THRESHOLD_MIB = parseInt(
  __ENV.LARGE_FILE_THRESHOLD_MIB || "256",
); // 256 MiB default
...
const errorReportLoaded = (function () {
  ...
  try {
    ...
    if (fileExists && ERROR_REPORT_FILE) {
      try {
        ...
        if (ERROR_REPORT_FILE.endsWith(".md")) {
          ...
          if (
            errorData.detailed_errors &&
            Array.isArray(errorData.detailed_errors)
          ) {
        ...
        console.log(
          `Loaded ${errorFiles.size} files to exclude from error report`,
        );
        return true;
      } catch (err) {
        // Only log once
    ...
    if (DEBUG_MODE) {
      console.warn(
        `Error report file ${ERROR_REPORT_FILE} not found or couldn't be accessed`,
      );
    }
    return false;
  }
  ...
  try {
      ...
      if (firstKey) {
        console.log(
          `DEBUG: First result structure: ${
            JSON.stringify(data.results[firstKey]).substring(0, 500)
          }...`,
        );
      }
    }
    ...
    Object.entries(data.results).forEach(([key, fileData]) => {
      ...
      if (fileData.derivatives) {
        Object.entries(fileData.derivatives).forEach(
          ([derivativeName, derivativeData]) => {
            // Skip invalid derivatives
            if (
              !derivativeData || derivativeData.status !== "success" ||
              derivativeData.status_code !== 200
            ) {
              if (DEBUG_MODE) {
                console.log(
                  `Skipping invalid derivative: ${key} (${derivativeName})`,
                );
              }
              return;
            }
            ...
            if (responseSize <= 1000) {
              if (DEBUG_MODE) {
                console.log(
                  `Skipping file with too small response size: ${path} (${derivativeName}) - ${responseSize} bytes`,
                );
              }
              return;
            }
    ...
    if (DEBUG_MODE) {
      console.log(
        `Processed ${filteredResults.length} valid videos from results file`,
      );
    }
  ...
  stages: [
    {
      ...
      target: parseInt(__ENV.STAGE1_USERS || "5"),
    },
    {
      duration: __ENV.STAGE2_DURATION || "1m",
      target: parseInt(__ENV.STAGE2_USERS || "10"),
    },
    {
      duration: __ENV.STAGE3_DURATION || "30s",
      target: parseInt(__ENV.STAGE3_USERS || "15"),
    },
    {
      duration: __ENV.STAGE4_DURATION || "1m",
      target: parseInt(__ENV.STAGE4_USERS || "10"),
    },
    {
      duration: __ENV.STAGE5_DURATION || "30s",
      target: parseInt(__ENV.STAGE5_USERS || "0"),
    },
  ],
  thresholds: {
    ...
    "http_req_duration{status:200}": [
      `p(95)<${__ENV.SUCCESS_DURATION_THRESHOLD || "12000"}`,
    ],
    "http_req_duration{status:206}": [
      `p(95)<${__ENV.PARTIAL_DURATION_THRESHOLD || "10000"}`,
    ],
    "http_req_duration{derivative:desktop}": [
      `p(95)<${__ENV.DESKTOP_DURATION_THRESHOLD || "15000"}`,
    ],
    "http_req_duration{derivative:tablet}": [
      `p(95)<${__ENV.TABLET_DURATION_THRESHOLD || "12000"}`,
    ],
    "http_req_duration{derivative:mobile}": [
...
function generateVideoUrl(videoInfo) {
  ...
  const path = videoInfo.path.startsWith("/")
    ? videoInfo.path.slice(1)
    : videoInfo.path;

  // Remove trailing slash from BASE_URL if it exists and path doesn't start with slash
  const baseUrl = BASE_URL.endsWith("/") ? BASE_URL.slice(0, -1) : BASE_URL;

  // Always use the imwidth format (no derivative parameter)
...
function getContentLength(videoUrl, headers = {}) {
  ...
  try {
    ...
    if (headResponse.status !== 200) {
      console.warn(
        `HEAD request failed for ${videoUrl}: status ${headResponse.status}`,
      );
      return null;
    }
    ...
    const contentLength = parseInt(
      (headResponse.headers && headResponse.headers["Content-Length"]) || "0",
    );
...
function generateRandomRange(videoInfo, videoUrl, headers = {}) {
  ...
  if (!contentSize && videoInfo && videoInfo.size && videoInfo.size > 1000) {
    ...
    if (DEBUG_MODE) {
      console.log(
        `Using estimated size ${formatFileSize(contentSize)} for ${videoUrl}`,
      );
    }
  } else if (!contentSize) {
  ...
  if (SIMULATE_REAL_PLAYER) {
    ...
    if (playbackProgress < 0.2) {
      ...
      return `bytes=${segmentPosition}-${
        Math.min(segmentPosition + chunkSize - 1, safeSize - 1)
      }`;
    } else {
      ...
      return `bytes=${startPos}-${
        Math.min(startPos + chunkSize - 1, safeSize - 1)
      }`;
    }
  } else {
  ...
  while (retries <= maxRetries) {
    try {
      ...
      if (response.status === 200 || response.status === 206) {
        ...
        if (isValid) {
          // Optional: Add a small delay to simulate connection handling
          // This is now disabled by default, can be enabled with CONNECTION_CLOSE_DELAY env var
          const connectionCloseDelay = parseInt(
            __ENV.CONNECTION_CLOSE_DELAY || "0", // Default to 0 (disabled)
          );
          if (connectionCloseDelay > 0) {
            if (DEBUG_MODE) {
              console.log(
                `Waiting ${connectionCloseDelay}s for connection to close`,
              );
            }
          ...
          if (DEBUG_MODE && response.status === 206) {
            ...
            console.log(
              `- Content-Length: ${
                response.headers["Content-Length"] ||
                response.headers["content-length"] || "not set"
              }`,
            );
            console.log(
              `- Content-Range: ${
                response.headers["Content-Range"] ||
                response.headers["content-range"] || "not set"
              }`,
            );
          }
          ...
          if (DEBUG_MODE) {
            console.log(
              `Warning: Response has ${response.status} status but appears invalid:`,
            );
            console.log(`- URL: ${url}`);
            console.log(`- Body size: ${bodySize} bytes`);
      ...
      if (DEBUG_MODE) {
        console.log(
          `Attempt ${retries + 1}/${
            maxRetries + 1
          } failed with status ${response.status} for ${url}`,
        );
...
function init() {
  ...
  console.log("╠════════════════════════════════════╦═══════════════════════╣");
  console.log(
    `║ Base URL                           ║ ${padRight(BASE_URL, 21)} ║`,
  );
  console.log(
    `║ Results File                       ║ ${
      padRight(shortenPath(RESULTS_FILE), 21)
    } ║`,
  );
  console.log(
    `║ Error Report File                  ║ ${
      padRight(shortenPath(ERROR_REPORT_FILE || "Not specified"), 21)
    } ║`,
  );
  console.log(
    `║ URL Format                         ║ ${padRight("imwidth", 21)} ║`,
  );
  console.log(
    `║ Max Retries                        ║ ${
      padRight(MAX_RETRIES.toString(), 21)
    } ║`,
  );
  console.log(
    `║ Available Videos                   ║ ${
      padRight(videoData.length.toString(), 21)
    } ║`,
  );
  console.log(
    `║ Simulate Real Player               ║ ${
      padRight(SIMULATE_REAL_PLAYER.toString(), 21)
    } ║`,
  );
  console.log(
    `║ Use HEAD Requests                  ║ ${
      padRight(USE_HEAD_REQUESTS.toString(), 21)
    } ║`,
  );
  console.log(
    `║ Skip Large Files                   ║ ${
      padRight(SKIP_LARGE_FILES.toString(), 21)
    } ║`,
  );
  console.log(
    `║ Large File Threshold               ║ ${
      padRight(LARGE_FILE_THRESHOLD_MIB + " MiB", 21)
    } ║`,
  );
  console.log(
    `║ Debug Mode                         ║ ${
      padRight(DEBUG_MODE.toString(), 21)
    } ║`,
  );
  console.log("╚════════════════════════════════════╩═══════════════════════╝");
  ...
  if (skippedFiles.errorFiles > 0 || skippedFiles.largeFiles > 0) {
    ...
    console.log(
      `║ Error Files Excluded               ║ ${
        padRight(skippedFiles.errorFiles.toString(), 21)
      } ║`,
    );
    console.log(
      `║ Large Files Excluded              ║ ${
        padRight(
          skippedFiles.largeFiles + " (>" + LARGE_FILE_THRESHOLD_MIB + " MiB)",
          21,
        )
      } ║`,
    );
    console.log(
      `║ Total Files Excluded              ║ ${
        padRight(
          (skippedFiles.errorFiles + skippedFiles.largeFiles).toString(),
          21,
        )
      } ║`,
    );
    ...
    if (DEBUG_MODE) {
      if (skippedFiles.errorFiles > 0) {
        ...
        if (skippedFiles.errorFilePaths.length > 10) {
          console.log(
            `  ... and ${skippedFiles.errorFilePaths.length - 10} more`,
          );
        }
      }
      ...
      if (skippedFiles.largeFiles > 0) {
        ...
        if (skippedFiles.largeFilePaths.length > 10) {
          console.log(
            `  ... and ${skippedFiles.largeFilePaths.length - 10} more`,
          );
        }
      }
    }
  ...
  if (videoData.length === 0) {
    ...
    if (videoData.length >= 3) {
      ...
      videoData.forEach((v) => {
        derivativeCounts[v.derivative] = (derivativeCounts[v.derivative] || 0) +
          1;
      });

      Object.entries(derivativeCounts).forEach(([derivative, count]) => {
        console.log(
          `  - ${derivative}: ${count} videos (${
            Math.round(count / videoData.length * 100)
          }%)`,
        );
      });
        ...
        if (count > 0) {
          console.log(
            `  - ${category}: ${count} videos (${
              Math.round(count / sizes.length * 100)
            }%)`,
          );
        }
      });
...
export default function () {
  ...
  if (videoCount === 0) {
    ...
    if (!__VU || __VU === 1) {
      console.log(
        "No valid videos found in results file. Run the pre-warmer first.",
      );
    }
    return;
  }
  ...
  if (useRangeHeader) {
    ...
    params.headers["Range"] = generateRandomRange(
      videoInfo,
      videoUrl,
      params.headers,
    );
  ...
  const checks = {};
  let isCurrentRequestFailed = false;
  ...
  checks["status is 200 or 206"] = (r) => {
    ...
    if (!result) {
      isCurrentRequestFailed = true; // Mark this request as failed
      ...
      if (DEBUG_MODE) {
        ...
        if (r.status === 0 && r.error) {
          console.log(
            `Status code error (with error info): status=${r.status}, error="${r.error}" for URL: ${videoUrl}`,
          );
      ...
      try {
        ...
        if (DEBUG_MODE) {
          console.log(`Error tracking derivative metric: ${e.message}`);
        }
      }
  ...
  checks["got response body"] = (r) => {
    ...
    if (!result) {
      ...
      if (DEBUG_MODE) {
        console.log(
          `Empty body error: body size=${bodySize} bytes, has valid headers=${hasValidHeader}, status=${r.status} for URL: ${videoUrl}`,
        );
        if (r.headers) {
  ...
  checks["response time reasonable"] = (r) => {
    ...
    if (!result) {
      ...
      if (DEBUG_MODE) {
        console.log(
          `Timeout error: ${r.timings.duration}ms exceeded threshold ${threshold}ms for URL: ${videoUrl}`,
        );
      }
  ...
  checks["content-type is video"] = (r) => {
    const contentType =
      (r.headers && (r.headers["Content-Type"] || r.headers["content-type"])) ||
      "";
  ...
  // Increment the general failure counter once if any check failed
  if (isCurrentRequestFailed) {
    failureCounter.add(1);
  }

  // Cache Content-Length for future requests if not already cached
  if (
    checks && response.headers && response.headers["Content-Length"] &&
    !CONTENT_LENGTH_CACHE.has(videoUrl)
  ) {
    ...
    if (contentLength > 0) {
      ...
      if (DEBUG_MODE) {
        console.log(
          `Cached Content-Length ${contentLength} (${
  ...
  if (DEBUG_MODE && response.status === 206) {
    ...
    let bodySize = 0;
    if (response.body) {
      if (typeof response.body.byteLength === "number") {
        bodySize = response.body.byteLength;
      } else if (typeof response.body.length === "number") {
        bodySize = response.body.length;
      } else if (typeof response.body.size === "number") {
    ...
    console.log(`- URL: ${videoUrl}`);
    console.log(
      `- Request Range header: ${params.headers["Range"] || "not specified"}`,
    );
    console.log(`- Response body size: ${bodySize} bytes`);
    console.log(
      `- Content-Range: ${
        response.headers["Content-Range"] ||
        response.headers["content-range"] || "not specified"
      }`,
    );
    console.log(`- Content-Type: ${contentType}`);
    ...
    try {
      console.log(
        `- Check results: status=${
          checks["status is 200 or 206"](response)
        }, body=${checks["got response body"](response)}, content-type=${
          checks["content-type is video"](response)
        }`,
      );
    } catch (e) {
...
export function handleSummary(data) {
  try {
    ...
    const totalFailures = data.metrics.custom_failures
      ? data.metrics.custom_failures.values.count
      : 0;
    ...
    let reportData = {
      ...
      test_params: {
        ...
        url_format: "imwidth", // Always using imwidth format
      ...
      failures: {
        total: totalFailures,
        by_type: {
          status_code: data.metrics.status_code_errors
            ? data.metrics.status_code_errors.values.count
            : 0,
          empty_body: data.metrics.empty_body_errors
            ? data.metrics.empty_body_errors.values.count
            : 0,
          timeout: data.metrics.timeout_errors
            ? data.metrics.timeout_errors.values.count
            : 0,
          content_type: data.metrics.content_type_errors
            ? data.metrics.content_type_errors.values.count
            : 0,
        },
        by_derivative: {
          desktop: data.metrics.desktop_failures
            ? data.metrics.desktop_failures.values.count
            : 0,
          tablet: data.metrics.tablet_failures
            ? data.metrics.tablet_failures.values.count
            : 0,
          mobile: data.metrics.mobile_failures
            ? data.metrics.mobile_failures.values.count
            : 0,
        },
        by_size: {
          small: data.metrics.small_file_failures
            ? data.metrics.small_file_failures.values.count
            : 0,
          medium: data.metrics.medium_file_failures
            ? data.metrics.medium_file_failures.values.count
            : 0,
          large: data.metrics.large_file_failures
            ? data.metrics.large_file_failures.values.count
            : 0,
        },
      },
    ...
    if (totalFailures > 0) {
      ...
      Object.entries(reportData.failures.by_type).forEach(([type, count]) => {
        if (count > 0) {
          ...
          console.log(
            `║  - ${type.padEnd(20)}: ${count.toString().padStart(5)} (${
              percent.padStart(4)
            }%)${" ".repeat(13)}║`,
          );
        }
      ...
      if (
        Object.values(reportData.failures.by_derivative).some((count) =>
          count > 0
        )
      ) {
        console.log(
        ...
        console.log(
          `║ FAILURES BY DERIVATIVE:                                 ║`,
        );
        Object.entries(reportData.failures.by_derivative).forEach(
          ([derivative, count]) => {
            if (count > 0) {
              const percent = (count / totalFailures * 100).toFixed(1);
              console.log(
                `║  - ${derivative.padEnd(20)}: ${
                  count.toString().padStart(5)
                } (${percent.padStart(4)}%)${" ".repeat(13)}║`,
              );
            }
          },
        );
      ...
      if (
        Object.values(reportData.failures.by_size).some((count) => count > 0)
      ) {
        console.log(
          `╠════════════════════════════════════════════════════════════╣`,
        );
        ...
        Object.entries(reportData.failures.by_size).forEach(([size, count]) => {
          if (count > 0) {
            const percent = (count / totalFailures * 100).toFixed(1);
            console.log(
              `║  - ${size.padEnd(20)}: ${count.toString().padStart(5)} (${
                percent.padStart(4)
              }%)${" ".repeat(13)}║`,
            );
          }
