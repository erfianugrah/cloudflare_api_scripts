...
def enable_dmarc(BASE_URL, headers):
    zone_ids = iterate_zone_ids_into_list(BASE_URL, headers)
    payload = {"enabled": True}  
    responses = []  
    for zone_id in zone_ids:
        dmarc_api = BASE_URL + f"/{zone_id}/email/security/dmarc-reports"
        response = requests.patch(dmarc_api, headers=headers, json=payload)
        responses.append(response)  
    return responses  

                
enable_dmarc(BASE_URL, headers)