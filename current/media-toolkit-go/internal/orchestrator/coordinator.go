...
func (c *Coordinator) executePrewarmWorkflow(workflowConfig WorkflowConfig, result *WorkflowResult) error {
	...
	filterConfig := &utils.FilterConfig{
		Extensions:         c.appConfig.Extensions,
		MediaType:          c.appConfig.MediaTypeString,
		ExcludeExtensions:  c.appConfig.ExcludeExtensions,
		ExcludePatterns:    c.appConfig.ExcludePatterns,
		ExcludeDirectories: c.appConfig.ExcludeDirectories,
		ExcludeMinSize:     c.appConfig.ExcludeMinSize,
		ExcludeMaxSize:     c.appConfig.ExcludeMaxSize,
	}
	objects = utils.ApplyFileFilters(objects, filterConfig)

	c.logger.Info("Files after filtering", zap.Int("count", len(objects)))
